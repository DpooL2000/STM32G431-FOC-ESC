...
// ==========================================
// UPDATED FLUX OBSERVER (Flux Weakening + Dynamic Gamma)
// ==========================================
void execute_Flux_Observer(float I_U, float I_V) {
  ...
  // Cache absolute speed to save CPU cycles
  ...
  // --- 1. FLUX WEAKENING (Softening Lambda) ---
  float effective_lambda = LAMBDA;
  if (abs_speed > 500.0f) {
      // Linearly reduce lambda by 2% for every 100 rad/s over 500
      float attenuation = 1.0f - (0.0002f * (abs_speed - 500.0f)); 
      
      // Hard floor at 60% so the observer doesn't collapse entirely
  ...
  // --- 2. DYNAMIC GAMMA (Noise Suppression at High Speed) ---
  float gamma = 10000.0f; 
  if (abs_speed > 500.0f) {
      // Relax the observer stiffness to ignore high-frequency ADC noise
      // Drops from 10000 down to a minimum of 2000
  ...
  // if (abs_speed > 5.0f) { 
  //     float phase_shift = get_Fast_Atan2(omega_c, abs_speed); 
  //     estimated_angle -= phase_shift * 0.5f; 
  // }
...
void loop() {
  ...
  // // --- MATHEMATICAL D/Q EXTRACTION FOR TELEMETRY ---
  // float I_alpha_tel = I_U;
  // float I_beta_tel  = -(I_U + 2.0f * I_V) * 0.577350269f;
  
  // float active_angle = estimated_angle;
  // if (current_state == STATE_ALIGN || current_state == STATE_OPEN_LOOP_RAMP) {
  //     active_angle = open_loop_angle;
  // }
  
  // float sin_p, cos_p;
  // get_Fast_Sin_Cos(active_angle, &sin_p, &cos_p);
  // float current_id = I_alpha_tel * cos_p + I_beta_tel * sin_p;
  // float current_iq = I_alpha_tel * sin_p - I_beta_tel * cos_p;
