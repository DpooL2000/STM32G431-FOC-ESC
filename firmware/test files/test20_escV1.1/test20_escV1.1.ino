...
// Active-Low 6N137 | Coasting | Classic BLDC Audio Engine
// =================================================================
#include <Arduino.h>
#include <SimpleFOC.h>
...
// ==========================================
// 5. BLDC AUDIO TONE GENERATOR (LOUD)
// ==========================================
void playTone(int freq_hz, int duration_ms) {
  ...
  // 12% power for a loud, punchy beep
  ...
  while (millis() - start_time < duration_ms) {
    // Pulse U High, while pulling V and W firmly to Ground. 
    // This turns the whole motor bell into a loud speaker cone.
    TIM1->CCR1 = beep_ccr; TIM1->CCR2 = 0; TIM1->CCR3 = 0;
    TIM1->CCER = base_ccer | TIM_CCER_CC1E | TIM_CCER_CC1NE | 
  ...
  TIM1->CCER = base_ccer; // Ensure safely off
}
...
void loop() {
  ...
  if (is_armed && elrs_throttle > 0.0f) {
    ...
    // STOPPED / DISARMED: Send sector 0 to COAST and prevent braking heat
