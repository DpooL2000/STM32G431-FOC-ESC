...
// ==========================================
// 8. MAIN LOOP (V1.5 - Auto-Advance Anti-Stall)
// ==========================================
void loop() {
  uint32_t now_us = micros();
  static uint32_t last_loop_us = 0;
  static float current_active_voltage = 0.0f; 
  static int sync_error_count = 0; // Tracks consecutive missed zero-crossings
  ...
  if (is_stalled) {
    if (millis() - stall_timer_ms > 2000) { 
      ...
      step_delay_us = 4000;               
      sync_error_count = 0; // Clear errors for restart
  ...
  if (!is_stalled) {
    ...
    if (is_armed && elrs_throttle > 0.0f) {
      ...
      // 3. --- SMART AUTO-ADVANCE WATCHDOG ---
      uint32_t dynamic_timeout = (closed_loop) ? (step_delay_us * 3) : 6000;
      if (closed_loop && dynamic_timeout < 1500) dynamic_timeout = 1500; // Enforce 1.5ms minimum grace period
      
      if (time_since_commutation > dynamic_timeout) {
          sync_error_count++; // Mark a strike
          
          if (sync_error_count > 4) {
             // 4 strikes in a row = The prop is physically jammed.
             setCommutation(0, 0.0f);        
             is_stalled = true;              
             ...
             // False Positive / Noise: Force the motor to step to keep it spinning
             current_sector++;
