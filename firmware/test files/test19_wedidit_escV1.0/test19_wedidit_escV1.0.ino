...
// Active-Low 6N137 | Coasting/Freewheel | Arming Tones
// =================================================================
#include <Arduino.h>
#include <SimpleFOC.h>
...
void setCommutation(int sector, float duty) {
  ...
  // If sector is 0, disable all MOSFETs (High Impedance COASTING)
  if (sector == 0) {
    TIM1->CCER = base_ccer; 
    return;
  }

  TIM1->CCER = base_ccer; // Disable briefly to prevent shoot-through
...
void playTone(int freq_hz, int duration_ms) {
  ...
  // 5% power to vibrate the coils without spinning
  uint16_t beep_ccr = (uint16_t)(0.05f * 4250.0f); 
  uint32_t base_ccer = TIM_CCER_CC1P | TIM_CCER_CC1NP | TIM_CCER_CC2P | TIM_CCER_CC2NP | TIM_CCER_CC3P | TIM_CCER_CC3NP;

  while (millis() - start_time < duration_ms) {
    // Pulse Phase U High, Phase V Low
  ...
  while (ELRS_Serial.available()) {
    ...
    if (crsf_idx >= 2 && crsf_idx >= crsf_buf[1] + 2) { 
        if (crsf_buf[2] == 0x16) { 
          ...
          // --- ARMING SEQUENCE & SOUNDS ---
          if (!is_armed && elrs_throttle == 0.0f) {
             is_armed = true;
             Serial.println("ARMING DETECTED! Playing Tones...");
             playTone(1046, 150); // C6
             delay(50);
             playTone(1318, 150); // E6
             delay(50);
             playTone(1567, 300); // G6
          }
        }
        crsf_idx = 0; 
    }
