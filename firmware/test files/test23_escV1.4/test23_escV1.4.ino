...
// Active-Low 6N137 | Advanced Dynamic Stall Protection & Recovery
// =================================================================
#include <Arduino.h>
#include <SimpleFOC.h>
...
// ==========================================
// 5. BLDC AUDIO TONE ENGINE (CK DEFINITION)
// ==========================================
...
void setup() {
  ...
  Serial.println("--- SIX-STEP ESC KERNEL V1.4 ---");
...
void loop() {
  ...
  if (is_stalled) {
    if (millis() - stall_timer_ms > 2000) { // Mandatory 2-second clearing window
      is_stalled = false;                 
      last_commutation_us = now_us;     
      current_active_voltage = 0.0f;      
      step_delay_us = 4000;               // Reset straight back to slowest spool speed
  ...
  if (!is_stalled) {
    ...
    if (is_armed && elrs_throttle > 0.0f) {
      ...
      // Lockout closed loop if we've recently experienced a recovery cycle
      ...
      if (!closed_loop) {
        ...
        float safe_spool_voltage = 1.5f + (spool_progress * 2.5f); // Upgraded curve
        target_voltage = min(safe_spool_voltage, requested_voltage);
      } else {
        target_voltage = requested_voltage;
      }

      // 2. HIGH-ACCELERATION SLEW RATE LIMITER
      if (current_active_voltage < target_voltage) {
        current_active_voltage += 120.0f * dt; // Accelerated to 120 V/s for agile response
      ...
      // 3. --- DYNAMIC WATCHDOG MONITOR ---
      // Timeout shrinks adaptively as speed increases (Timeout = 4x predicted step duration)
      uint32_t dynamic_timeout = (closed_loop) ? (step_delay_us * 4) : 6000;
      
      if (time_since_commutation > dynamic_timeout) {
          setCommutation(0, 0.0f);        // Cut power immediately (no inductive slam)
          is_stalled = true;              
          ...
          zc_detected = false;  
          current_active_voltage = 0.0f;           
          Serial.print("SYNC LOSS DETECTED! Dynamic Timeout Breached at: "); 
          Serial.print(dynamic_timeout); Serial.println("us. Disengaging FETs.");
          return;
      }
