...
const float RS = 0.093f;      
// const float LS = 0.0000468f;   
const float LAMBDA = 0.00036f; 
const float LS     = 0.000033668f;
...
// --- THE FIX: Sequential Read Safety ---
...
void execute_Flux_Observer(float I_U, float I_V) {
  float I_alpha = I_U;
  float I_W = I_V; // Renaming for sanity: ADC2 is physically Phase W
  
  // The negative sign in front perfectly corrects the Phase W hardware swap
...
void setup() {
  hardware_lockdown();
  Serial.begin(2000000); // MAX BAUD for Binary Stream
  delay(1000);

  // --- THE FIX: CORDIC Initialization ---
...
void loop() {
  ...
  uint32_t start_cycles = DWT->CYCCNT; // START STOPWATCH
  ...
  switch (current_state) {
    ...
    case STATE_ALIGN:
      {
        float align_v = (float)tick_counter / 10000.0f * 1.5f; // Boosted Align Voltage to 1.5V
    ...
    case STATE_OPEN_LOOP_RAMP:
      {
        // 1. Gentle acceleration to 300 rad/s
        if (open_loop_speed < 300.0f) open_loop_speed += 50.0f * LOOP_DT; 
        else open_loop_speed = 300.0f;
        ...
        // 2. Open-loop voltage push
        float v_mag = 1.0f + (open_loop_speed * 0.005f); 
        cmd_v_alpha = -v_mag * sin_ol;
        cmd_v_beta  =  v_mag * cos_ol;
        setInverterVoltages(cmd_v_alpha, cmd_v_beta);

        // 3. INFINITE SPOOL TEST
        // No timer. No handoff to FOC. Just hold open-loop.
        // We do this to watch the Observer (Green) track the Command (Yellow).
  ...
  uint32_t end_cycles = DWT->CYCCNT; // STOP STOPWATCH
