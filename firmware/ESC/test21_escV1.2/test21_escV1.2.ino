...
// Active-Low 6N137 | Coasting | Audio Engine | STALL RECOVERY
// =================================================================
#include <Arduino.h>
#include <SimpleFOC.h>
...
const uint32_t WATCHDOG_TIMEOUT_US = 5000; // 5ms maximum allowable step time in closed-loop
...
void loop() {
  ...
  if (is_stalled) {
    if (millis() - stall_timer_ms > 500) {
      is_stalled = false;                 // Clear the stall flag after 500ms
      last_commutation_us = micros();     // Reset the commutation timer
      Serial.println("Attempting Restart...");
    } else {
      setCommutation(0, 0.0f);            // Ensure FETs stay off during wait
  ...
  if (is_armed && elrs_throttle > 0.0f) {
    ...
    if (closed_loop && time_since_commutation > WATCHDOG_TIMEOUT_US) {
        setCommutation(0, 0.0f);        // Cut power to save MOSFETs instantly
        is_stalled = true;              // Flag the state machine
        stall_timer_ms = millis();      // Start the 500ms recovery wait
        closed_loop = false;            // Drop out of closed loop
        step_delay_us = 4000;           // Reset to slow open-loop spooling
        zc_detected = false;            
    ...
    closed_loop = false;
    step_delay_us = 4000; 
    last_commutation_us = now_us; // Keep timer fresh so we don't trip watchdog on startup
  }
  ...
  if (millis() - last_print_ms > 100) { 
    ...
    Serial.print("Mode:"); Serial.print(is_stalled ? "STALL" : (closed_loop ? "CLOSED" : "OPEN"));
