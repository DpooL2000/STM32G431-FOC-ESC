...
// // --- MOTOR DNA ---
// const float RS     = 0.025f;        // 25 milliohms
// const float LS     = 0.00000375f;   // 3.75 microhenries
// const float LAMBDA = 0.00079f;     // We will leave flux linkage here for now
    ...
    case STATE_OPEN_LOOP_RAMP:
      {
        ...
        // 1. Spool up to 805 ~ 1100rpm rad/s
    ...
    case STATE_FOC_BLEND:
      {
        ...
        float MIN_VQ = 0.75f; // Exactly matches your 500.0f * 0.0015f
        float MAX_VQ = 11.0f;  
        float target_vq = MIN_VQ + (elrs_throttle * (MAX_VQ - MIN_VQ));

        // Create a multiplier that goes from 0.0 to 1.0 over 1s
        float blend_factor = (float)blend_timer / 20000.0f;
        
        // LERP: Smoothly slide from the open-loop voltage to the transmitter voltage
        ...
        open_loop_angle = closed_loop_angle; // Keeps GUI lines locked

        // When the 100ms fade is done, drop into raw FOC
        if (blend_timer >= 20000) {
            current_state = STATE_CLOSED_LOOP_FOC;
            blend_timer = 0; // Reset for next arming cycle
        }
    ...
    case STATE_CLOSED_LOOP_FOC:
      {
        ...
        // Pure, unfiltered, zero-latency RC command for flight
