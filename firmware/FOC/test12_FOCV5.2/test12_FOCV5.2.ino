// =================================================================
// BARE-METAL FOC MASTER KERNEL V4.2 (PURE VOLTAGE + D/Q TELEMETRY)
...
// --- MOTOR DNA ---
const float RS     = 0.090f;
const float LS     = 0.00003f;
const float LAMBDA = 0.00041f;

float offset_U = 2048.0f;
float offset_V = 2048.0f;

enum DriveState { STATE_OFF, STATE_ALIGN, STATE_OPEN_LOOP_RAMP, STATE_FOC_BLEND, STATE_CLOSED_LOOP_FOC };
...
float closed_loop_angle = 0.0f;   

// Trackers for the Telemetry Plotter
float active_vd = 0.0f;
float active_vq = 0.0f;
...
// --- ULTIMATE 49-BYTE TELEMETRY STRUCT ---
...
// ==========================================
// UPDATED FLUX OBSERVER (Alpha-Beta Flywheel)
// ==========================================
void execute_Flux_Observer(float I_U, float I_V) {
  ...
  float abs_speed = fabs(estimated_speed);

  float omega_c = abs_speed * 0.10f;
  ...
  float effective_lambda = LAMBDA;
  if (abs_speed > 500.0f) {
      float attenuation = 1.0f - (0.0002f * (abs_speed - 500.0f)); 
      if (attenuation < 0.60f) attenuation = 0.60f; 
      effective_lambda = LAMBDA * attenuation;
  }
  float target_flux_sq = effective_lambda * effective_lambda;
  float flux_err = target_flux_sq - current_flux_sq;

  float gamma = 10000.0f; 
  if (abs_speed > 500.0f) {
      gamma = 10000.0f - (4.0f * (abs_speed - 500.0f));
      if (gamma < 2000.0f) gamma = 2000.0f; 
  }
  ...
  // --- THE DIGITAL FLYWHEEL (Alpha-Beta Tracking Filter) ---
  static float filter_angle = 0.0f;

  // 1. Predict Future Angle based on Momentum
  float predicted_angle = filter_angle + (estimated_speed * LOOP_DT);
  if (predicted_angle >= _2PI) predicted_angle -= _2PI;
  if (predicted_angle < 0.0f) predicted_angle += _2PI;

  // 2. Measure the Error vs the Noisy ArcTan
  float angle_error = raw_estimated_angle - predicted_angle;
  if (angle_error > _PI) angle_error -= _2PI;
  if (angle_error < -_PI) angle_error += _2PI;

  // 3. Apply Alpha and Beta Gains
  const float ALPHA_GAIN = 0.2f;     // Position stiffness
  const float BETA_GAIN  = 2000.0f;  // Speed tracking stiffness

  filter_angle = predicted_angle + (ALPHA_GAIN * angle_error);
  if (filter_angle >= _2PI) filter_angle -= _2PI;
  if (filter_angle < 0.0f) filter_angle += _2PI;

  estimated_speed = estimated_speed + (BETA_GAIN * angle_error * LOOP_DT);
  estimated_angle = filter_angle; 
}
...
void loop() {
  ...
  static float close_loop_angle = 0.0f;
  ...
  // // 2. READ CURRENTS (With Zero-Lag Slew Rate Limiter)
  // float I_U_raw = (float)((int32_t)ADC1->JDR1 - (int)offset_U) * NOMINAL_AMPS_PER_COUNT;
  // float I_V_raw = (float)((int32_t)ADC2->JDR1 - (int)offset_V) * NOMINAL_AMPS_PER_COUNT;

  // static float prev_IU = 0.0f;
  // static float prev_IV = 0.0f;
  // const float MAX_DELTA_I = 10.0f; // Max physical amp change per 50us loop

  // // Clamp the delta to kill high-frequency spikes instantly
  // float delta_U = constrain(I_U_raw - prev_IU, -MAX_DELTA_I, MAX_DELTA_I);
  // float delta_V = constrain(I_V_raw - prev_IV, -MAX_DELTA_I, MAX_DELTA_I);

  // float I_U = prev_IU + delta_U;
  // float I_V = prev_IV + delta_V;
  ...
  switch (current_state) {
    ...
    case STATE_ALIGN:
      {
        ...
        active_vd = align_v; active_vq = 0.0f;
    ...
    case STATE_OPEN_LOOP_RAMP:
      {
        ...
        // Spool up to 500 rad/s
        if (open_loop_speed < 500.0f) {
            open_loop_speed += 5000.0f * LOOP_DT; 
            ramp_hold_timer = 0; 
        } 
        else {
            open_loop_speed = 500.0f;
            ramp_hold_timer++;   
        }
        ...
        float v_mag = (open_loop_speed * 0.0015f); 
        cmd_v_alpha = v_mag * sin_ol;
        cmd_v_beta  = -v_mag * cos_ol;
        setInverterVoltages(cmd_v_alpha, cmd_v_beta);

        active_vd = 0.0f; active_vq = v_mag;

        if (ramp_hold_timer > 2000) {
            current_state = STATE_FOC_BLEND;
            ramp_hold_timer = 0; 
        }
    ...
    case STATE_FOC_BLEND:
      {
        static uint32_t blend_timer = 0;
        blend_timer++;

        float MIN_VQ = 0.75f; 
        float MAX_VQ = 11.0f;  
        float target_vq = MIN_VQ + (elrs_throttle * (MAX_VQ - MIN_VQ));

        float blend_factor = (float)blend_timer / 20000.0f;
        cmd_vq_throttle = MIN_VQ + ((target_vq - MIN_VQ) * blend_factor);
        ...
        active_vd = 0.0f; active_vq = cmd_vq_throttle;

        closed_loop_angle = estimated_angle;
        open_loop_angle = closed_loop_angle; 

        if (blend_timer >= 20000) {
            current_state = STATE_CLOSED_LOOP_FOC;
            blend_timer = 0; 
        }

        if (elrs_throttle == 0.0f) {
    ...
    case STATE_CLOSED_LOOP_FOC:
      {
        ...
        // --- PILLAR A: PREDICTIVE PHASE ADVANCE ---
        // CPU processing + PWM timer delay is roughly 40 microseconds.
        // We aim the voltage where the rotor WILL be, not where it was.
        float phase_advance = estimated_speed * 40e-6f; 
        float advanced_angle = estimated_angle + phase_advance;
        
        if (advanced_angle > _2PI) advanced_angle -= _2PI;
        if (advanced_angle < 0.0f) advanced_angle += _2PI;

        float sin_cl, cos_cl;
        get_Fast_Sin_Cos(advanced_angle, &sin_cl, &cos_cl);
        
        // --- PILLAR D: D-AXIS FEEDFORWARD ---
        // 1. Do a quick Park transform just to find the Torque Current (Iq)
        float I_alpha = I_U;
        float I_beta  = -(I_U + 2.0f * I_V) * 0.577350269f;
        float current_iq = I_alpha * sin_cl - I_beta * cos_cl; 

        // 2. Calculate the rogue cross-coupling voltage (V = -omega * L * Iq)
        float active_vd_ff = -estimated_speed * LS * current_iq;

        // 3. Apply the Inverse Park Transform using BOTH Vq (throttle) and Vd (cancellation)
        // This matches your specific coordinate system mapping (Vq aligned to sin/-cos)
        cmd_v_alpha = (active_vd_ff * cos_cl) + (cmd_vq_throttle * sin_cl);
        cmd_v_beta  = (active_vd_ff * sin_cl) - (cmd_vq_throttle * cos_cl);
        
        setInverterVoltages(cmd_v_alpha, cmd_v_beta);

        active_vd = active_vd_ff; 
        active_vq = cmd_vq_throttle;
  ...
  // 6. EXACT 49-BYTE BINARY TELEMETRY DUMP
  tick_counter++;
  // if (tick_counter % 20 == 0) { 
  //   t_data.loop_cycles = end_cycles - start_cycles;
  //   t_data.i_alpha     = I_alpha_tel;
  //   t_data.i_beta      = I_beta_tel;
  //   t_data.id_measured = current_id;
  //   t_data.iq_measured = current_iq;
  //   t_data.id_target   = 0.0f;       
  //   t_data.iq_target   = 0.0f;       
  //   t_data.v_d         = active_vd;
  //   t_data.v_q         = active_vq;
